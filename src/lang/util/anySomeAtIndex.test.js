import anySomeAtIndex from './anySomeAtIndex'

describe('anySomeAtIndex', () => {
  test('returns true when value in array asserts true', () => {
    const array = ['a', 'b', 'c']
    const result = anySomeAtIndex((val, index) => val === 'b' && index === 1, 0, array)
    expect(result).toBe(true)
  })

  test('throws when passed a value that is not array like', () => {
    expect(() => anySomeAtIndex(() => true, 0, {})).toThrow(
      expect.objectContaining({
        message: expect.any(String),
        type: 'UnexpectedType'
      })
    )
    expect(() => anySomeAtIndex(() => true, 0, null)).toThrow(
      expect.objectContaining({
        message: expect.any(String),
        type: 'UnexpectedType'
      })
  ...
  test('returns false if no value is matched', () => {
    const array = ['a', 'b', 'c']
    const result = anySomeAtIndex((val) => val === 'd', 0, array)
  ...
  test('starts at the given index', () => {
    ...
    const result = anySomeAtIndex(predicate, 1, array)
  ...
  test('stops when a true assertion is found', () => {
    ...
    const result = anySomeAtIndex(predicate, 0, array)
  ...
  test('starts at 0 if the length minus the given index is still negative', () => {
    const array = ['a', 'b', 'c']
    const predicate = jest.fn((value) => value === 'd')
    const result = anySomeAtIndex(predicate, -4, array)
    expect(predicate).toHaveBeenNthCalledWith(1, 'a', 0)
    expect(predicate).toHaveBeenNthCalledWith(2, 'b', 1)
