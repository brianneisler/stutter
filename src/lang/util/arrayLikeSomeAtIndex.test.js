import arrayLikeSomeAtIndex from './arrayLikeSomeAtIndex'

describe('arrayLikeSomeAtIndex', () => {
  test('returns true when value in array asserts true', () => {
    const array = ['a', 'b', 'c']
    const result = arrayLikeSomeAtIndex(array, (val, index) => val === 'b' && index === 1, 0)
    expect(result).toBe(true)
  })

  test('returns false if no value is matched', () => {
    const array = ['a', 'b', 'c']
    const result = arrayLikeSomeAtIndex(array, (val) => val === 'd', 0)
  ...
  test('starts at the given index', () => {
    ...
    const result = arrayLikeSomeAtIndex(array, predicate, 1)
  ...
  test('stops when a true assertion is found', () => {
    ...
    const result = arrayLikeSomeAtIndex(array, predicate, 0)
  ...
  test('starts at 0 if the length minus the given index is still negative', () => {
    const array = ['a', 'b', 'c']
    const predicate = jest.fn((value) => value === 'd')
    const result = arrayLikeSomeAtIndex(array, predicate, -4)
    expect(predicate).toHaveBeenNthCalledWith(1, 'a', 0)
    expect(predicate).toHaveBeenNthCalledWith(2, 'b', 1)
  ...
  test('upgrades to a Promise when an async predicate is used', async () => {
    const array = ['a', 'b', 'c']
    let result = arrayLikeSomeAtIndex(
      array,
      (val, index) =>
        new Promise((resolve) => {
          setTimeout(() => {
